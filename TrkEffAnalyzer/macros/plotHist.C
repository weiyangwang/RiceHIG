void plotHist() {
  ...
  //gROOT->Macro("$CMSSW_BASE/src/UserCode/ASYoon/dNdEtaPlotMacrosV2/paper/rootlogon.C");

  gStyle->SetOptStat(0);

 // set_plot_style();

  TFile *f = new TFile("/mnt/hadoop/cms/store/user/davidlw/merged_hydjet_442.root");
  ...
  TH2F *hAcc = (TH2F*) f->Get("trkEffAnalyzer/hacc"); hAcc->GetYaxis()->SetRangeUser(0,10);
  TH2F *hEff = (TH2F*) f->Get("trkEffAnalyzer/heff"); hEff->GetYaxis()->SetRangeUser(0,10);
  TH2F *hMul = (TH2F*) f->Get("trkEffAnalyzer/hmul"); hMul->GetYaxis()->SetRangeUser(0,10);
  ...
  legEta->AddEntry(gAccEta,"p_{T} > 0.4 GeV/c","lp");
...
//  gEffPt2->BayesDivide(hEffPt2,hAccPt2);
  ...
  c7->cd(1); c7->GetPad(1)->SetLogx(); hDumEtaEff->Draw(); gEffEta->Draw("pc"); gEffEta2->Draw("pc"); legEta->Draw();
  c7->cd(2); c7->GetPad(2)->SetLogx(); hDumPtEff->Draw(); gEffPt->Draw("pc"); gEffPt2->Draw("pc"); legPt->Draw();
  TFile* fout = new TFile("hydjet_44x.root","recreate");
  gEffEta->Write();
  gEffEta2->Write();
  gEffPt->Write();
  gEffPt2->Write();
